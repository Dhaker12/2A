...
try{
	...
	$page=$_GET['nom'];
    $handle = $link->prepare("select nick ,kills, kda from team where clan='$page' "); 
    $handle->execute(); 
    $result = $handle->fetchAll(\PDO::FETCH_OBJ);
		
    foreach($result as $row){
        array_push($dataPoints1, array("label"=> $row->nick, "y"=> $row->kills));
        array_push($dataPoints2, array("label"=> $row->nick, "y"=> $row->kda));
    }
	$link = null;
}
...
var chart = new CanvasJS.Chart("chartContainer", {
	...
	theme: "light2",
	title:{
		text: "stats over the players of team : <?php echo $page; ?> "
	},
	legend:{
		cursor: "pointer",
